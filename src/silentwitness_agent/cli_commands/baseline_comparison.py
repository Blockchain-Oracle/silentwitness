...
def _resolve_dataset_id(case_dir: Path) -> str:
    """Read case manifest if present; otherwise treat the case_id as the dataset_id."""
...
def _gt_path(dataset_id: str) -> Path:
    return Path(__file__).resolve().parents[3] / "harness" / "ground_truth" / f"{dataset_id}.json"
...
def _compute_delta_dict(
    ...
    sw = json.loads(silentwitness_path.read_text())
    scoring = json.loads(scoring_path.read_text()) if scoring_path else {}

    b_metrics = dict(scoring.get("baseline", {}))
    s_metrics = dict(scoring.get("silentwitness", {}))
    # Fallback to runner-side timing pass-through
    b_t = b_metrics.get("time_to_handoff_ready_report_seconds") or baseline.get(
        "time_to_handoff_ready_report_seconds"
    )
    s_t = s_metrics.get("time_to_handoff_ready_report_seconds") or sw.get(
        "time_to_handoff_ready_report_seconds"
    )
...
def _resolve_metric_filter(metrics_arg: str) -> tuple[str, ...]:
    """Parse --metrics comma-list, resolving aliases."""
...
def run(
    ...
    *,
    ...
    dataset_id = _resolve_dataset_id(case_dir)
    ...
    case_results = results_dir or (case_dir / "results")
    if not case_results.exists():
        case_results = Path("harness") / "results" / dataset_id
    ...
    shown = _resolve_metric_filter(metrics_arg)
