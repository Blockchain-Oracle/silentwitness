...
def test_dlllist_against_nist_image(tmp_path: Path) -> None:
    case_dir = tmp_path / "case-nist-dlllist"
    case_dir.mkdir()
    logger = AuditLogger(case_dir, examiner="aj")
    registry = EvidenceRegistry(case_dir=case_dir)
...
def test_handles_against_nist_image(tmp_path: Path) -> None:
    case_dir = tmp_path / "case-nist-handles"
    case_dir.mkdir()
    logger = AuditLogger(case_dir, examiner="aj")
    registry = EvidenceRegistry(case_dir=case_dir)
    registry.register(_NIST_FIXTURE, EvidenceType.MEMORY_DUMP, audit_id="sift-aj-20260610-001")
