...
def test_audit_store_unwritable_when_append_fails(
    ...
    real_append = __import__(
        "silentwitness_mcp.findings.pivot", fromlist=["append_chained_jsonl"]
    ).append_chained_jsonl

    def _raise_on_hypothesis(path: Path, record: dict[str, object]) -> dict[str, object]:
        if path.name == "hypothesis.jsonl":
            raise OSError("simulated disk full")
        return real_append(path, record)

    monkeypatch.setattr(
        "silentwitness_mcp.findings.pivot.append_chained_jsonl", _raise_on_hypothesis
    )
    payload = PivotInput(
        from_hypothesis_id="H-001",
        to_hypothesis_id="H-002",
...
def test_audit_write_failure_preserves_original_rejection(
    ...
    def _raise_on_findings(path: Path, record: dict[str, object]) -> dict[str, object]:
        if path.name == "findings.jsonl":
            raise OSError("simulated disk full")
        return real_append(path, record)

    monkeypatch.setattr("silentwitness_mcp.findings.pivot.append_chained_jsonl", _raise_on_findings)
    payload = PivotInput(
        from_hypothesis_id="H-999",
        to_hypothesis_id="H-002",
        reason=_VALID_REASON,
