# ROCBA OpenAI GPT-5.4 mini clean showcase

This directory is a point-in-time snapshot copied from the VPS case workspace:

```text
/root/silentwitness-demo/cases/rocba-gpt-5-4-mini-001
```
...
- Approved finding records: `4`
- Draft finding records: `0`
- Report frontmatter status: `REVIEWED`

The `ERROR` status is preserved intentionally. The investigator confirmed four
...
| `verification/rocba-gpt-5-4-mini-001.jsonl` | HMAC approval ledger records. |
| `approval-results.json` | Programmatic approval results for the DRAFT findings. |
| `status.txt` | Exact `silentwitness status rocba-gpt-5-4-mini-001` output at capture. |
