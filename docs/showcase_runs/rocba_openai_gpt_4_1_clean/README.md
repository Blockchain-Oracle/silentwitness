# ROCBA OpenAI GPT-4.1 clean showcase

This directory is a point-in-time snapshot copied from the VPS case workspace:

```text
/root/silentwitness-demo/cases/rocba-gpt-4-1-001
```
...
- Case: `rocba-gpt-4-1-001`
- Dataset: ROCBA / Standard Forensic Case
- Model: `openai-chat:gpt-4.1`
- Token use at capture: `4M / 6M`
- Investigation status at capture: `ERROR`
- Hypotheses confirmed: `5`
- Approved finding records: `6`
- Draft finding records: `0`
- Report frontmatter status: `REVIEWED`

The `ERROR` status is preserved intentionally. The investigator completed five
confirmed hypotheses and produced a reviewed report, but the process exited
non-zero after the narrative/finalization phase. The artifacts here are not
edited to hide that state.
...
| `findings.json` | Observation and finding records captured by the run. |
...
| `verification/rocba-gpt-4-1-001.jsonl` | HMAC approval ledger records. |
| `approval-results.json` | Programmatic approval results for the DRAFT findings. |
| `status.txt` | Exact `silentwitness status rocba-gpt-4-1-001` output at capture. |
