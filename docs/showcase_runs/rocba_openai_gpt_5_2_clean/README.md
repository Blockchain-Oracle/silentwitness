# ROCBA OpenAI GPT-5.2 clean showcase

This directory is a point-in-time snapshot copied from the VPS case workspace:

```text
/root/silentwitness-demo/cases/rocba-gpt-5-2-001
```
...
- Case: `rocba-gpt-5-2-001`
- Dataset: ROCBA / Standard Forensic Case
- Model: `openai-chat:gpt-5.2`
- Token use at capture: `1M / 6M`
- Investigation status at capture: `MAX_ITERATIONS`
- Hypotheses confirmed: `1`
- Approved finding records: `2`
- Draft finding records: `0`
- Report frontmatter status: `REVIEWED`

The `MAX_ITERATIONS` status is preserved intentionally. The investigator
confirmed one hypothesis and produced a reviewed report, then stopped on the
configured iteration cap. The artifacts here are not edited to hide that state.
...
| `verification/rocba-gpt-5-2-001.jsonl` | HMAC approval ledger records. |
| `approval-results.json` | Programmatic approval results for the DRAFT findings. |
| `status.txt` | Exact `silentwitness status rocba-gpt-5-2-001` output at capture. |
