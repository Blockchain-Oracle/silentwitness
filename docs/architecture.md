...
This document defines the components, their interfaces, the trust boundaries between them, and the architectural mechanisms that make hallucinated findings impossible to record.
...
|---|---|---|---|
| Python runtime | CPython | `>=3.12,<3.14` | SIFT 2026 ships Python 3.12 on Ubuntu 24.04.2 Noble. 3.13 forward-compat is tested in the CI matrix. |
...
| Report PDF | `weasyprint` | `>=68.1,<70.0` | Pattern reuse from Protocol SIFT. **Pin floor closes CVE-2025-68616** (was `>=60,<62` — affected). Native deps cairo/pango/gdk-pixbuf + `libharfbuzz-subset0`, `libpangoft2-1.0-0`, font packages documented in the Dockerfile. |
...
| Pre-commit | `pre-commit` | `>=4` | Standard. Configured via `.pre-commit-config.yaml`. |
| File-size guard | local custom hook | (local) | Enforces ≤400 LOC/file. |
| Secret detection | `detect-secrets` | `>=1.5` | Baseline committed at `.secrets.baseline`. |
...
- **Claude Agent SDK** — locks provider to Anthropic. Pydantic AI's agent-delegation primitive is the provider-agnostic equivalent (Pydantic AI library research §13).
...
Package layout with file-purpose comments. Every Python file is bound by the ≤400-LOC pre-commit guard.
...
├── docs/                                  # judge-facing docs + diagrams
...
This is the artifact judges score on Constraint Implementation and Audit Trail Quality. It is the **product**. Everything else (`silentwitness-agent`, `silentwitness-claude-code`) is a reference consumer.
...
`cmdline`, `netscan`, `psscan`) for process and network corroboration. The `targeted` profile adds
`malfind --pid` for a bounded PID set selected from network-owning processes, suspicious command
lines/process names, and `psscan`-only processes. The `deep` profile runs the expensive
all-process malware VAD scanner (`malfind`). Each plugin has visible CLI progress and a bounded
timeout (`SILENTWITNESS_VOL3_TIMEOUT_SEC`, default 300 seconds, with plugin-specific overrides such
as `SILENTWITNESS_VOL3_TIMEOUT_MALFIND_SEC`). A slow or failed plugin becomes an audit advisory
while successful memory rows remain searchable.
...
Every MCP tool call emits one JSONL line to `cases/<case_id>/audit/<backend>.jsonl`, where `<backend>` is the tool family (`memory`, `disk`, `log`, `network`, `registry`, `findings`). The schema:
...
4. **Dangerous Unicode.** Strip:
   - **RLO / LRO / BIDI controls:** U+202A through U+202E, U+2066 through U+2069 (Boucher & Anderson "Trojan Source" — LLM failure-modes threat model §3.7).
   - **Zero-width characters:** U+200B (ZWSP), U+200C (ZWNJ), U+200D (ZWJ), U+FEFF (ZWNBSP).
   - **Tag characters:** U+E0000 through U+E007F (Riley Goodside 2024 demonstrated frontier models decode these — LLM failure-modes threat model §3.7).
...
The sanitizer is **not** a complete defense against prompt injection — no sanitizer is (LLM failure-modes threat model §3.6, Yotam Perkal's "structural > prompt" position). The architectural defenses against PI are the citation + entity gates: the model cannot record an observation citing injected content because the injected content's hash will not match a registered tool output, and any entity it tries to claim will not appear in cited spans.

### 4.9 HMAC-signed approval ledger

Per the Valhuntir pattern (HMAC ledger + bidirectional reconciliation).
...
- **System prompt.** The senior-analyst frame. Verbatim (lives in `silentwitness_agent/prompts/investigator.md`):
  ...
  Notably absent: "Ralph Wiggum Loop," "court-admissible," "autonomous SOC," "find evil." Vocabulary discipline enforced by the CI grep gate.

### 5.2 Specialists (Pydantic AI agent-delegation)

Four specialists, each its own `Agent`, invoked from the investigator via the agent-delegation pattern (Pydantic AI library research §2 — "agent delegation in Pydantic AI propagates dependencies and usage automatically").
...
`hypothesis/stack.py` — plain Python, no framework (~200 LOC budget).
...
The pivot count secondary metric is computed by `grep -c '"type":"pivot"' cases/<case_id>/audit/hypothesis.jsonl`.
...
The critic does **not** approve findings (only the examiner does, via HMAC ledger). It is an internal quality gate, not an authorization step. This matches Valhuntir's L1 structural approval gate (Valhuntir comparative analysis §2 L1: "the LLM cannot approve its own findings even if it tried").

### 5.6 CLI (Typer)

Command names only here; full ergonomics live in `silentwitness --help`.
...
The drop-in `.claude/` config is the zero-setup judge convenience. SIFT 2026 ships Claude Code v2.0.61 at `/usr/local/bin/claude` per SIFT 2026 tool-catalog research.
...
7. Optionally install Velociraptor binary if `--with-velociraptor` flag is set (stretch — out of v1 scope).
8. Add PECmd + SrumECmd EZ Tools via the `dotnet` wrapper pattern (SIFT 2026 tool-catalog research — EZ Tools missing from saltstack).
...
The Compose file mounts `/evidence:ro,noexec,nosuid` (the mount validator catches it if the host volume is wrong), mounts `./cases` as a writable volume, and binds `/var/lib/silentwitness` to a named Docker volume so the ledger survives container restarts. The image is pulled from GHCR.

### 7.3 MCP transports

- **stdio (default).** Claude Code launches `python -m silentwitness_mcp` as a subprocess. Communication over the subprocess's stdin/stdout per MCP spec (MCP protocol spec §A3.1). No port. Trivial security boundary.
- **Streamable HTTP on `localhost:4508`.** Available via `silentwitness serve --http` for remote MCP clients. Binds `127.0.0.1` only (DNS-rebinding defense per MCP protocol spec §A3.2). Bearer token auth from `$SILENTWITNESS_GATEWAY_TOKEN` env var. Optional; not required for the demo.

### 7.4 Optional HUD

`localhost:8088` if `silentwitness serve --hud` is set. Port 8088 explicitly, NOT 80 (Apache binds CyberChef on port 80 on SIFT 2026). The HUD is a stretch deliverable — Server-Sent-Events streaming agent traces. Out of v1 commitment; documented here for completeness.
...
For each threat: definition, our architectural mitigation, residual risk. Source cites LLM failure-modes threat model.
...
The threat model is residual-aware: not every threat is eliminated. The architecture comprehends each one with awareness of which are mitigated, which are tolerated, and which are residual (LLM failure-modes threat model §14 closing).
...
- **Context.** The hackathon overview names four approaches. Direct Agent Extension means writing only Claude-Code skills + prompts on top of pre-installed Claude Code. Custom MCP Server means building a typed tool surface the agent dispatches against. The judging rubric weights Constraint Implementation heavily, and the overview names approach #2 verbatim as "the most sound architecture."
...
- **Context.** The agent loop must be model-agnostic. The Claude Agent SDK is Anthropic-only.
...
- **Consequences.** Pydantic AI is newer (17.5K stars, daily commits — Pydantic AI library research §2). API may shift; we pin `pydantic-ai==1.105.*` in `uv.lock`. We use agent-delegation for specialists rather than a SDK-native handoff primitive.
...
- **Alternatives considered.** LLM-rendered HTML (re-renders are non-deterministic; diffing is unreliable; Protocol SIFT's per-case generated Python script pattern is fragile per Valhuntir comparative analysis §1.7). Direct PDF via reportlab (lower fidelity; editing is impossible). DOCX (template hell; opaque diffs).
...
- **Consequences.** No extra dep. Matches Valhuntir's pattern verbatim (Valhuntir comparative analysis §2 L2). Constant-time comparison via `hmac.compare_digest`.
...
- **Context.** Valhuntir runs 9 separate MCP backends behind a gateway (Valhuntir comparative analysis §2). Each backend is a separate package, separate process, separate state machine.
...
- **Context.** Judges may want to test on a model that doesn't require API credits. The model-agnostic contract requires CI-testing against ≥2 providers.
...
- **Consequences.** Quality drops on smaller local models — documented honestly in the accuracy report per Rob T. Lee's honesty rubric. The architecture is unchanged; the agent loop is provider-agnostic.
...
Every dep is added explicitly. Reuse existing libraries; do not reinvent.
...
The `uv.lock` is committed; CI runs `uv sync --frozen`.
...
CI enforces these via `ruff` rules and the custom file-size guard; pre-commit catches them locally. Implementing agents must NOT use:
...
CI gates are defined under `.github/workflows/`. The architecture relies on the following gates being present and green:

- **File-size guard:** ≤400 LOC per `.py` file. CI re-runs the pre-commit guard against all tracked files.
- **mypy strict** on `src/`.
- **Test coverage:** ≥85% overall, ≥95% on `src/silentwitness_mcp/verification/`, ≥90% on `src/silentwitness_mcp/audit/`.
- **No banned patterns in `src/`.** Ruff rule set enforces.
- **SBOM + license-check + dependency-review + Trivy** — supply-chain hygiene the threat model relies on.
- **Property tests on the verification gates** via `hypothesis`. Coverage of the gates includes adversarial-input property tests (random Unicode, random injection patterns, malformed audit IDs).
...
5. **Multi-case archive.** Single-case in v1. The directory layout supports multi-case (each at `cases/<case_id>/`), but no aggregation CLI in v1.
